    ...
    <section className="space-y-6">
      {backHref ? (
        <div className="flex flex-col gap-3 rounded-3xl border border-border/70 bg-background/80 p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <Link
            href={backHref}
            className="inline-flex items-center gap-2 self-start rounded-full border border-border bg-card px-4 py-2 text-sm font-medium text-foreground transition hover:border-foreground/20 hover:bg-muted/60"
          >
            <ArrowLeft className="h-4 w-4" />
            {backLabel || "Back"}
          </Link>
          <p className="text-sm leading-6 text-muted-foreground">
            Return to the dashboard at any time. Your import tools stay available from the top
            navigation.
          </p>
        </div>
      ) : null}
